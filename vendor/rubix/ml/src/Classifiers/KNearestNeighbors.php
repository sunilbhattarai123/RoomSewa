...
class KNearestNeighbors implements Estimator, Learner, Online, Probabilistic, Persistable
{
   ...
   public function nearest(array $sample): array
   {
       // Initialize an array to store distances.
       $distances = [];
   
       // Calculate distances between the input sample and each neighbor in the dataset.
       foreach ($this->samples as $index => $neighbor) {
           // Compute the distance and store it in the $distances array with the key as the neighbor's index.
           $distances[$index] = $this->kernel->compute($sample, $neighbor);
       }
   
       // Sort distances in ascending order while maintaining key associations.
       asort($distances);
   
       // Select the K nearest neighbors by taking the first $this->k elements.
       $selectedDistances = array_slice($distances, 0, $this->k, true);
   
       // Retrieve the labels of the nearest neighbors using the selected distances.
       $labels = array_intersect_key($this->labels, $selectedDistances);
   
       // Return the labels, distances, and keys of the K nearest neighbors.
       return [$labels, $selectedDistances, array_keys($selectedDistances)];
   }
